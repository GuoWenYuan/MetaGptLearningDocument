...
class GameArchitectDiagramAction(Action):
    """生成架构图"""
    async def run(self, technical_documentation):
        prompt = self._get_prompt()
        prompt = prompt.replace(Const.Document,technical_documentation)
        prompt = prompt.replace(Const.DesignDoc,ShareBlackBoard.get_doc(Const.DesignDoc))
        result = await self._aask(prompt)
        return self._save_architecture_diagram(result)

    def _save_architecture_diagram(self, content):
        """保存架构图"""
        # 提取架构图信息
        pattern = r'```plantuml(.*)```'
        ...
        if not match:
            ...
            match = re.search(pattern, content, re.DOTALL)
            content = match.group(1) if match else content
        ShareBlackBoard.save_doc(Const.ArchitectureDiagram, content)
        logger.info(content)
        return content
...
asyncio.run(GameArchitectDiagramAction().run(doc))